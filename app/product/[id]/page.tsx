...
export async function generateMetadata({ params }: ProductPageProps): Promise<Metadata> {
  ...
  if (!product) return { title: 'Product Not Found — OpenSourceRadar' };

  return {
    title: `${product.title} — ${product.tagline} | OpenSourceRadar`,
      ...
      <div className="sidebar-card">
        ...
        <div style={{ fontSize: '0.85rem', color: 'var(--text-body)' }}>
          <div style={{ marginBottom: '0.6rem' }}>
            ...
            <strong style={{ color: 'var(--text-ink)' }}>{config?.name || 'OpenSourceRadar'}</strong>
          </div>
          <div>
            <span style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '1px', color: 'var(--text-muted)', fontWeight: 600 }}>Niche:</span>{' '}
            <span style={{ color: 'var(--text-body)' }}>{config?.tags?.join(', ') || 'open-source'}</span>
          </div>
        </div>
      </div>
      <AdSlot ad={ad} />
